	...
	for pot_map_idx in range(len(pots_selected)):
		...
		for outcome_idx_map in outcome_idx_map_set:
			...
			for i_selector,selector in enumerate(stim_selector_set):
				...
				Msel = copy.deepcopy(Mlist[selector])
				Fsel = copy.deepcopy(Flist[selector])
...
with open(Path.home() / 'Desktop' / 'conditions_9.json', 'w', encoding='utf-8') as f:
	json.dump(counts['index'].tolist(), f, ensure_ascii=False, indent='\t', separators=(',', ':'))

### print stim dict
with open(Path.home() / 'Desktop' / 'stim_9.json', 'w', encoding='utf-8') as f:
