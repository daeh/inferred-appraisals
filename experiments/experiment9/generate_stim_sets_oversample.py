...
while continue_randomization and loop_count < 1e6:
	...
	Mlist_new = np.array(copy.copy(stim_base[:-4:-2]))
	Flist_new = np.array(copy.copy(stim_base[-2:-5:-2]))
		
	Mlist = np.array(copy.copy(stim_base[:-4:2]))
	Flist = np.array(copy.copy(stim_base[1:-4:2]))

	assert len(Flist) == len(Flist)
	# assert all M and all F

	np.random.shuffle(Mlist_new)
	np.random.shuffle(Flist_new)
	...
	for pot_map_idx in range(len(pots_selected)):
		...
		for outcome_idx_map in outcome_idx_map_set:
			...
			for i_selector,selector in enumerate(stim_selector_set):
				...
				Msel = [copy.deepcopy(Mlist_new[new_idx]), *copy.deepcopy(Mlist[selector])]
				Fsel = [copy.deepcopy(Flist_new[new_idx]), *copy.deepcopy(Flist[selector])]
	...
	for i_list,stimlist in enumerate(stim_set):
		for i_stim,stim in enumerate(stimlist):
			...
			counts['outcome'][i_list,i_stim] = f'{a1}'
			counts['pot'][i_list,i_stim] = stim['pot']
			counts['genderXoutcome'][i_list,i_stim] = f"{stim['pronoun']}{a1}"
			counts['stimXoutcome'][i_list,i_stim] = f"{stim['stimulus']}{a1}"
			counts['stimXoutcomeXpot'][i_list,i_stim] = f"{stim['stimulus']}{a1}{stim['pot']}"
...
with open(Path.home() / 'Desktop' / 'conditions_9_unbalanced.json', 'w', encoding='utf-8') as f:
	json.dump(counts['index'].tolist(), f, ensure_ascii=False, indent='\t', separators=(',', ':'))

### print stim dict
with open(Path.home() / 'Desktop' / 'stim_9_unbalanced.json', 'w', encoding='utf-8') as f:
